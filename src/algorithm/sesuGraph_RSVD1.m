function F_star = sesuGraph_RSVD1(Y, X, alpha, sigma, nystroemFraction)
...
[V_W,D_W] = rsvd(W_mm,m,p,q);

%%%% form the approximation

U_W = W_nm * ( sqrt(m/n) * V_W );
D_W = (n/m) * diag(diag(D_W).^-1);

d_n = abs(sum(U_W) * D_W * U_W.');
...
%S_mm = (S_mm + S_mm) / 2; 
%Enforce symmetry - would otherwise not be given because of extremely small numerical differences
...
[V_S,D_S] = rsvd(S_mm,m,p,q);                                              clear S_mm;

%%%% form the approximation

U_S = S_nm * ( sqrt(m/n) * V_S );                                          clear S_nm;
